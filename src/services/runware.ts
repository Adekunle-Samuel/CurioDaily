...
export const generateContextualImage = async (title: string, topic: string, apiKey?: string): Promise<string | null> => {
  if (!apiKey) return null;
  
  try {
    const service = new RunwareService(apiKey);
    const prompt = `High-quality illustration for ${topic} topic: ${title}. Educational, clean, modern style, vibrant colors, detailed, professional`;
